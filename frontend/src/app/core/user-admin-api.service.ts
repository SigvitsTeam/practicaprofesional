...
export interface ManagedUserRecord {
  id: string; fullName: string; email: string; phone?: string; active: boolean; hasExternalIdentity: boolean;
  role: { code: string; name: string; startDate: string };
  assignment: { scopeType: string; regionId?: string; municipalityId?: string; facilityId?: string; label: string; startDate: string };
  updatedAt: string;
}
...
export class UserAdminApiService {
  ...
  private get endpoint() { return `${this.config.apiUrl}/v1/admin/users`; }
  list() { return this.http.get<ManagedUserRecord[]>(this.endpoint); }
  create(input: { fullName: string; email: string; phone?: string; roleCode: string; scopeType: string; regionId?: string; municipalityId?: string; facilityId?: string; startDate: string; reason: string }) { return this.http.post<ManagedUserRecord>(this.endpoint, input); }
