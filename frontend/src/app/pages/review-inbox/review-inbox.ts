...
export class ReviewInbox implements OnInit {
  ...
  reload() {
    ...
    if (role === 'municipal-coordinator') {
      this.api
        ...
        .subscribe({
          next: (reports) => {
            this.liveReports.set(reports.map((report) => this.toTableReport(report)));
            this.loading.set(false);
          },
          error: () => {
            this.liveReports.set([]);
            this.loading.set(false);
            this.loadError.set('No fue posible cargar la bandeja municipal.');
          },
        });
      return;
    }
    if (role === 'regional-admin' || role === 'regional-superadmin') {
      this.api
        ...
        .subscribe({
          next: (reports) => {
            this.liveReports.set(reports.map((report) => this.toMunicipalTableReport(report)));
            this.loading.set(false);
          },
          error: () => {
            this.liveReports.set([]);
            this.loading.set(false);
            this.loadError.set('No fue posible cargar la bandeja regional.');
          },
        });
      return;
    }
    if (role === 'central-validator') {
      this.api
        ...
        .subscribe({
          next: (reports) => {
            this.liveReports.set(reports.map((report) => this.toRegionalTableReport(report)));
            this.loading.set(false);
          },
          error: () => {
            this.liveReports.set([]);
            this.loading.set(false);
            this.loadError.set('No fue posible cargar la bandeja central.');
          },
        });
      return;
    }
  ...
  get reports() {
    ...
    return q
      ? this.sourceReports.filter((item) => item.name.toLowerCase().includes(q))
      : this.sourceReports;
  }
  get entityLabel() {
    const role = this.roleContext.activeRoleId();
  ...
  count(status: Report['status']) {
    ...
    if (role === 'central-validator')
      return (
        { Aprobado: 0, 'En revisión': 1, Devuelto: 0, Pendiente: 17 } as Record<
          Report['status'],
          number
        >
      )[status];
    if (role === 'regional-admin' || role === 'regional-superadmin')
      return (
        { Aprobado: 1, 'En revisión': 1, Devuelto: 0, Pendiente: 10 } as Record<
          Report['status'],
          number
        >
      )[status];
  ...
  private toTableReport(report: Its2WorkflowReport): Report {
    ...
    return {
      ...
      sent: report.sentAt ? new Date(report.sentAt).toLocaleString('es-HN') : '—',
    };
  }
