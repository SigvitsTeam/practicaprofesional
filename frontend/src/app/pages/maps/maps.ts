...
import { ItsCaptureApiService, TerritorialAnalyticsLevel } from '../../core/its-capture-api.service';
...
export class Maps implements OnInit {
  ...
  setLevel(level: MapLevel) { this.mapLevel = level; this.metric = 'total'; this.load(); }
  resetFilters() { this.metric = 'total'; this.notify.emit('Filtros del mapa restablecidos.'); }

  private load() {
    if (this.auth.isDemo()) return;
    const level = ({ national: 'REGION', regional: 'MUNICIPIO', municipal: 'ESTABLECIMIENTO' } as const)[this.mapLevel] satisfies TerritorialAnalyticsLevel;
    this.loading.set(true); this.loadError.set('');
    this.api.getTerritorialAnalytics(level, 2026, 8).pipe(
      takeUntilDestroyed(this.destroyRef), finalize(() => this.loading.set(false)),
    ).subscribe({
      next: result => this.liveReports.set(result.rows.map(row => ({
        workflowId: row.reportId,
        workflowLevel: level === 'REGION' ? 'regional' : level === 'MUNICIPIO' ? 'municipal' : 'facility',
      ...
      error: () => { this.liveReports.set([]); this.loadError.set('No fue posible cargar los indicadores territoriales reales.'); },
    });
  }

  private reportStatus(status: string): Report['status'] {
    if (status.startsWith('APROBADO') || ['CONSOLIDADO_NACIONAL', 'CERRADO_OFICIAL'].includes(status)) return 'Aprobado';
